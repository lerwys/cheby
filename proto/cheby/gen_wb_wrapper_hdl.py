"""Create HDL Wishbone Wrapper from the tree"""
...
from cheby.hdltree import (HDLModule,
                           ...
                           HDLChoiceExpr, HDLChoiceDefault,
                           HDLBool, bit_0, bit_1, bit_x)
from cheby.layout import ilog2

mode_suffix_map = {'IN': '_i',
                   'OUT': '_o',
                   'INOUT': '_b',
                   'input': '_i',
                   'output': '_o',
                   'inout': '_b'}
rev_mode_suffix_map = {'OUT': '_i',
                       'IN': '_o',
                       'INOUT': '_b',
                       'output': '_i',
                       'input': '_o',
                       'inout': '_b'}
comp_mode_map = {'input': 'IN',
                 'output' : 'OUT',
                 'inout' : 'INOUT'}

RAM_ADDR_IN_PORT_SUFFIX = '_addr_i'
RAM_DATA_OUT_PORT_SUFFIX = '_data_o'
RAM_RD_IN_PORT_SUFFIX = '_rd_i'
RAM_DATA_IN_PORT_SUFFIX = '_data_i'
RAM_WR_IN_PORT_SUFFIX = '_wr_i'
RAM_BWSEL_IN_PORT_SUFFIX = '_bwsel_in'
...
def expand_rams(root, module, bus, isig):
    ...
    for r in rams:
        ...
        prefix = get_hdl_prefix(root) + '_' + get_hdl_prefix(r)
        addr = HDLPort(prefix + RAM_ADDR_IN_PORT_SUFFIX, r.c_sel_bits, dir='IN')
        addr.comment = "Ports for RAM: {}".format(r.description)
        # Connect this port to
        dato = HDLPort(prefix + RAM_DATA_OUT_PORT_SUFFIX, reg.width, dir='OUT')
        dato.comment = "Read data output"
        rd_sig = HDLPort(prefix + RAM_RD_IN_PORT_SUFFIX, dir='IN')
        ...
        if access_dev in ['READ_WRITE']:
            dati = HDLPort(prefix + RAM_DATA_IN_PORT_SUFFIX, reg.width, dir='IN')
            dati.comment = "Write data input"
            wr_sig = HDLPort(prefix + RAM_WR_IN_PORT_SUFFIX, dir='IN')
            wr_sig.comment = "Write strobe (active high)"
            r.code.ports.extend([dati, wr_sig])
            byte_select = get_wbgen(r, 'byte_select')
            if byte_select:
                bwsel_sig = HDLPort(prefix + RAM_BWSEL_IN_PORT_SUFFIX, reg.width // 8)
...
def expand_hdl(root, mod, vname):
    ...
    # Add all signals not marked as "special" to top-module
    mod_sig = {}
    for s in vmod.ports:
        if not s.annotations:
            mod_sig_name = s.name + mode_suffix_map[s.mode]
            mod_sig[s.name] = HDLSignal(mod_sig_name, size=s.size, typ='S' if s.sign=='signed' else 'L')
    ...
    # Construct to-be wrapped module
    wrap_inst = HDLInstance(vmod.name + "_inst", vmod.name)
    m.stmts.append(wrap_inst)
    # Add all signals not marked with annotations directly to
    # the top-module and all ports marked with annotations to
    # the Wishbone module
    for s in vmod.ports:
        if not s.annotations:
            wrap_inst.conns.append((s.name, mod_sig[s.name]))
    ...
    if irq_code.ports:
        ...
        wb_slave_inst.conns.append((bus_int.name, bus_int))
        ...
        if isinstance(r, tree.Reg) \
           or is_wbgen_fifocs(r):
            for port in r.code.ports:
                ...
                wb_slave_isigs[port.name] = \
                    HDLSignal(port_name_int, size=port.size, typ=port.typ)
            ...
            for port in r.code.ports:
                ...
                wb_slave_isigs[port.name] = \
                    HDLSignal(port_name_int, size=port.size, typ=port.typ)
                wb_slave_inst.conns.append((port.name, wb_slave_isigs[port.name]))
                # TODO. Add IRQ connection to wrapped module
        elif is_wbgen_irq(r):
            for irq in get_wbgen(r, 'irqs'):
                for port in irq['irq']['code'].ports:
                    port_name_int = port.name.replace('_i', '').replace('_o', '').replace('_b', '')
                    wb_slave_isigs[port.name] = \
                        HDLSignal(port_name_int, size=port.size, typ=port.typ)
                    wb_slave_inst.conns.append((port.name, wb_slave_isigs[port.name]))
                # TODO. Add FIFO connection to wrapped module
        elif is_wbgen_ram(r):
            for port in rams_code.ports:
                port_name_int = port.name.replace('_i', '').replace('_o', '').replace('_b', '')
                wb_slave_isigs[port.name] = \
                    HDLSignal(port_name_int, size=port.size, typ=port.typ)
                wb_slave_inst.conns.append((port.name, wb_slave_isigs[port.name]))
                # Connect remaining RAM ports to wrapped module: data and rd
                mod_port_name = re.sub(mode_suffix_map[port.dir], '', re.sub(get_hdl_prefix(root) + '_', '', port.name))
                ...
                if re.search(RAM_DATA_OUT_PORT_SUFFIX + '$', port.name):
                    ...
                    m.stmts.append(HDLAssign(wb_slave_isigs[port.name], bit_1))
            rams_code.ports = []

    #wb_slave.asgn_code.append(wb_slave_inst)

    # Generate wb_slave internal signals
    m.decls.extend(wb_slave_isigs.values())

    return m

def generate_wb_wrapper_hdl(root, mod, vname):

    module = expand_hdl(root, mod, vname)
    return module
